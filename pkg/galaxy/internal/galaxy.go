...
type Galaxy struct {
	...
	Thickness      float64
	Radius         float64 // ly
	Neighbourhoods []interfaces.StellarNeighbourhoodInterface
}
...
*/
func (g *Galaxy) GenerateStellarNeighbourhood() interfaces.StellarNeighbourhoodInterface {
	hzInner, hzOuter := g.HabitableZone()

	dist := g.Rng.Float64()*((hzOuter-hzInner)/100) + hzInner
	rad := float64(g.Rng.Intn(30-15) + 15)                  // 15 LY min, up to 30 LY
	sDensity := g.Rng.Float64()*((0.006-0.003)/100) + 0.003 // 0.003 min density, up to 0.006
	...
	neighbourhood := &StellarNeighbourhood{
		...
		Radius:  rad,
		Density: sDensity,
		Coords:  utils.RandomCartesianCoord(dist),
	}
